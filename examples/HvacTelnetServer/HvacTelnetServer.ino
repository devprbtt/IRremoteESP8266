...
String pageHeader(const String &title) {
  ...
  html += "<style>";
  html += ":root{--bg:#0d1117;--card:#161b22;--border:#30363d;--text:#c9d1d9;--muted:#8b949e;--accent:#58a6ff;--success:#238636;}";
  html += "body{margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;background:var(--bg);color:var(--text);line-height:1.5;}";
  html += ".wrap{max-width:1000px;margin:0 auto;padding:20px;}";
  html += "nav{display:flex;gap:8px;margin-bottom:24px;padding-bottom:12px;border-bottom:1px solid var(--border);overflow-x:auto;}";
  html += "nav a{color:var(--text);text-decoration:none;font-size:14px;font-weight:500;padding:6px 12px;border-radius:6px;transition:background .2s;white-space:nowrap;}";
  html += "nav a:hover{background:var(--border);}";
  html += "h2,h3,h4{margin:16px 0 8px;font-weight:600;color:var(--accent);text-transform:uppercase;letter-spacing:.5px;font-size:16px;}";
  html += ".card{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:20px;margin-bottom:20px;}";
  html += "label{font-size:13px;font-weight:500;color:var(--muted);display:block;margin-bottom:4px;}";
  html += "input,select,textarea{width:100%;box-sizing:border-box;padding:8px 12px;margin-bottom:12px;background:#010409;color:var(--text);border:1px solid var(--border);border-radius:6px;font-size:14px;}";
  html += "input:focus,select:focus,textarea:focus{outline:none;border-color:var(--accent);box-shadow:0 0 0 3px rgba(88,166,255,.3);}";
  html += "button{cursor:pointer;border:1px solid var(--border);border-radius:6px;padding:8px 16px;font-size:14px;font-weight:600;transition:.2s;display:inline-flex;align-items:center;justify-content:center;gap:8px;background:#21262d;color:var(--accent);}";
  html += "button:hover{background:#30363d;border-color:#8b949e;}";
  html += "button[type='submit'],button.primary{background:var(--success);color:#fff;border-color:rgba(240,246,252,.1);}";
  html += "button[type='submit']:hover,button.primary:hover{background:#2ea043;}";
  html += "input:disabled,select:disabled,textarea:disabled,button:disabled{opacity:.5;cursor:not-allowed;}";
  html += ".row{display:flex;align-items:center;gap:8px;margin-bottom:12px;}";
  html += ".row input[type='checkbox']{width:auto;margin:0;}";
  html += "table{border-collapse:collapse;width:100%;margin-bottom:16px;}th,td{border:1px solid var(--border);padding:10px;text-align:left;font-size:14px;}";
  html += "th{background:#161b22;color:var(--muted);font-weight:600;text-transform:uppercase;font-size:12px;}";
  html += "code,pre{background:#010409;border:1px solid var(--border);border-radius:6px;padding:12px;display:block;white-space:pre-wrap;font-family:ui-monospace,SFMono-Regular,SF Mono,Menlo,Consolas,monospace;font-size:13px;color:#79c0ff;}";
  html += ".grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px;}";
  html += ".pill{display:inline-block;background:rgba(88,166,255,.1);color:var(--accent);padding:2px 10px;border-radius:999px;font-size:12px;font-weight:500;}";
  html += "@media(max-width:600px){.wrap{padding:12px;}table{display:block;overflow-x:auto;}.grid{grid-template-columns:1fr;}}";
...
void handleFirmwarePage() {
  ...
  html += "<button type='submit' id='fwBtn' class='primary'>Upload & Flash</button></form>";
  html += "<div style='margin-top:20px;'><label>Upload Progress</label>";
  html += "<div style='height:10px;background:#21262d;border-radius:5px;margin:12px 0;overflow:hidden;border:1px solid var(--border);'><div id='fwBar' style='height:100%;background:var(--success);width:0%;transition:width 0.3s;'></div></div>";
  html += "<div id='fwStatus' style='font-size:12px;color:var(--muted);'>Idle</div></div>";
  ...
  html += "const fwStatus=document.getElementById('fwStatus');const fwBtn=document.getElementById('fwBtn');";
  ...
  html += "fwBtn.disabled=true;document.getElementById('fwBar').style.width='0%';fwStatus.textContent='Uploading...';";
  html += "xhr.upload.onprogress=(ev)=>{if(!ev.lengthComputable)return;const p=Math.round((ev.loaded/ev.total)*100);document.getElementById('fwBar').style.width=p+'%';fwStatus.textContent='Uploading '+p+'%';};";
  html += "xhr.onerror=()=>{fwBtn.disabled=false;fwStatus.textContent='Upload failed.';fwStatus.style.color='#f85149';};";
  html += "xhr.onload=()=>{fwBtn.disabled=false;if(xhr.status>=200&&xhr.status<300){document.getElementById('fwBar').style.width='100%';fwStatus.textContent='Upload complete. Processing update...';fwStatus.style.color='var(--success)';document.open();document.write(xhr.responseText);document.close();}else{fwStatus.textContent='Update failed ('+xhr.status+').';fwStatus.style.color='#f85149';}};";
  html += "xhr.send(fd);});}";
  html += "</script>";
  html += "</div>";
  html += pageFooter();
