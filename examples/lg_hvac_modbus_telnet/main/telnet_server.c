...
static bool handle_command(telnet_server_t *srv, int fd, char *line)
{
    ...
    if (strcasecmp(cmd, "HELP") == 0) {
        sendf(fd,
              "{\"ok\":true,\"commands\":[\"help\",\"status\",\"zones\",\"get <idx>\",\"read <idx>\",\"set <idx> <power|mode|fan|setpoint> <value>\",\"config get\",\"config set wifi <ssid> <pass>\",\"config set zones <csv>\",\"config set poll_ms <ms>\",\"config set idu_base <0|1>\",\"config set slave <1..247>\",\"config save\",\"reboot\",\"quit\"]}\r\n");
        return true;
    }
...
{
    sendf(fd, "Welcome to LG HVAC Modbus Telnet API. Type HELP.\r\n");
