...
void app_config_set_defaults(app_config_t *cfg)
{
    ...
    strlcpy(cfg->system.device_id, "lg-hvac-modbus-bridge", sizeof(cfg->system.device_id));
...
esp_err_t app_config_load(app_config_t *cfg)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(nvs_get_str_into(h, "ota_url", cfg->ota_url, sizeof(cfg->ota_url)));
...
esp_err_t app_config_to_json(const app_config_t *cfg, char *out, size_t out_len)
{
    ...
    size_t offset = 0;
    esp_err_t err = appendf(out, out_len, &offset,
                            ...
                            "\"hvac\":{\"idu_address_base\":%d,\"poll_interval_ms\":%d,\"mode_rate_limit_ms\":%d,\"setpoint_min_tenths\":%d,\"setpoint_max_tenths\":%d,\"zones\":[",
    ...
    for (size_t i = 0; i < cfg->hvac.zone_count; i++) {
        err = appendf(out, out_len, &offset, "%s{\"slave\":%u,\"central_address\":%u}",
                      (i == 0) ? "" : ",",
    ...
    return appendf(out, out_len, &offset, "]},\"ota_url\":\"%s\"}", cfg->ota_url);
}
